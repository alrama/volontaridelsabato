...
if ($conn->connect_error) {
  ...
  if ($nodo = $resultSQL->fetch_assoc()) {
      ...
      $sql = "SELECT data_evento from paniniweb_evento where gruppi_id = " . $gruppiID ;
      $resultSQL = $conn->query($sql);
      if ($nodo = $resultSQL->fetch_assoc()) {
        $result->response_code = 320;
        $result->error_description = "Prossima distribuzione già definita";
      } else {
        $sql = "insert into paniniweb_evento (data_evento,gruppi_id) VALUES ('".$_GET["evento"]."',".$gruppiID.")";
        $rc = $conn->query($sql);
        if (TRUE===$rc) {
