...
if ($conn->connect_error) {
  ...
  if ($nodo = $resultSQL->fetch_assoc()) {
      ...
      $sql = "insert into paniniweb_users (email,nome,cognome,gruppi_id,admin) VALUES ('".$mail."','".$_GET["nome"]."','".$_GET["cognome"]."',".$gruppiID.",".$_GET["admin"].")";
      $rc = $conn->query($sql);
      if (TRUE===$rc) {
	      $result->response_code = 200;
      } else {
        $result->response_code = 302;
        $result->error_description = "Errore nell'inserimento del volontario: " . $sql;
      }
    } else {
